...
class NewsFragmentTest {
    ...
    fun getHeadlineNews_Success() {
        ...
        onView(withText("Parade Planet Tak Akan Memengaruhi Cuaca - Medcom.Id"))
            .check(matches(isDisplayed()))
        onView(withId(R.id.rv_news))
            .perform(
                RecyclerViewActions.scrollTo<RecyclerView.ViewHolder>(
                    hasDescendant(withText("NASA Temukan Bintang Kejora Lain seperti Venus di Planet Kembaran Bumi | antariksa - Republika"))
                )
            )
    }
